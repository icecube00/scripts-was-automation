...
set CONFIG_DIR_NAME=%UTIL_DIR%\_start_server
set LOG_FILE=%mydate%_%mytime%_cluster_start.log
...
if exist %DMGR_HOME%\bin\serverStatus.bat (
 call %DMGR_HOME%\bin\serverStatus.bat -all -profileName %DMGR_PROFILE% %userParams% |find "STARTED" && goto:startCluster
) else (
 call:SCRIPTEND "[Ошибка] Не найден файл консоли IBM WebSphere: %DMGR_HOME%\bin\serverStatus.bat" exit
)
call:writetolog "Пытаемся выполнить запуск менеджера развертывания - DMGR"
if exist %DMGR_HOME%\bin\startManager.bat (
 call %DMGR_HOME%\bin\startManager.bat %userParams% >>%LOG_DIR%\%LOG_FILE% 2>&1
) else (
 call:SCRIPTEND "[Ошибка] Не найден файл запуска консоли IBM WebSphere: %DMGR_HOME%\bin\startManager.bat" exit
)
call:writetolog "Пытаемся выполнить запуск агентов узлов"
if exist %NODE_AGENT_HOME%\bin\startNode.bat (
 call %NODE_AGENT_HOME%\bin\startNode.bat %userParams% >>%LOG_DIR%\%LOG_FILE% 2>&1
) else (
 call:SCRIPTEND "[Ошибка] Не найден файл запуска агентов узлов IBM WebSphere: %NODE_AGENT_HOME%\bin\startNode.bat" exit
)

:startCluster
call:writetolog "Выполняем запуск узлов кластера"
...
call:writetolog "Работа скрипта запуска кластера завершена"
if "%~2"=="exit" (
